...
**Family:** Kernel · **Layer:** L0 · **SKU tier:** `included` · **Depends on:** KRN-01 (tenant/entity scope), KRN-06 (event bus/event store — `P-08 Event.reversal_handle` is written here, in the same transaction as the mutation), KRN-10 (audit — every reversal is itself audited)
...
owning module's compensation logic, not by KRN-18 directly.
...
| `status` | enum | `previewing` \| `confirmed` \| `executing` \| `completed` \| `partially_completed` \| `failed` |
| `irreversible_items` | list | `{reversal_handle_id, reason, proposed_correction_action}` |
| `confirmed_at`, `confirmed_by` | | Distinct from `initiated_by` where a second approver is required (§11) |
...
- **"Undo this agent's day"** — PR-01, PR-21: select an agent (and
  ...
  (reversible items, irreversible items with proposed corrections), confirm
  or cancel.
...
| POST | `/api/v1/core/reversals/{batch_id}/confirm` | Transitions `previewing → confirmed → executing`; rejects if the batch's underlying scope has changed since preview (KRN-18-FR-005) |
| GET | `/api/v1/core/reversals/{batch_id}` | Batch status and `reversal_log` entries |
...
agent-day operation), `read_audit`.

| Persona | single_reverse (own record) | batch_preview | batch_confirm (own scope) | batch_confirm_agent_day | read_audit |
|---|---|---|---|---|---|
| PR-01 Owner | ✓ | ✓ | ✓ | ✓ | ✓ |
| PR-21 System Admin | ✓ | ✓ | ✓ | ✓ (delegated per KRN-05 matrix) | ✓ |
| PR-16 CFO | ✓ (financial records) | ✓ (financial scope) | ✓ (financial scope) | ✗ | ✓ |
...
3. **Second-approver requirement on `batch_confirm_agent_day`** — this draft
   assumes PR-01/PR-21 alone can confirm a whole-agent-day reversal (§11),
   with no explicit second-approver/maker-checker step, since neither Vol 0
   §27.3 nor Vol 1's KRN-18 section requires one. Given the financial
   consequence a large batch could carry, confirm whether KRN-05's approval
   matrix should be invoked here for batches above a declared value ceiling,
   the same way agent financial ceilings work (Vol 0 §27.2/§27.3).
